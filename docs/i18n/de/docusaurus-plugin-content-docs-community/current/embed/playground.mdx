...
export const EmbedPlayground = () => { const [host, setHost] = useState("https://butterfly.linwood. ev") const [speichern, setSave] = useState(true) const [bearbeitbar, setEditable] = useState(true) const [theme, setTheme] = useState("user") const [language, setLanguage] = useState("user") const url = `${host}/embed? ave=${save}&editable=${editable}&theme=${theme}&language=${language}`; return (
      <div>
