...
| ----------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Szín        | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Áttetszőség | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Áttetszőség | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Property | Default  | Leírás                                      |
| -------- | -------- | ------------------------------------------- |
| Indítás  | `(0, 0)` | Start of the gradient axis. |
...
| Property     | Default              | Leírás                                                                        |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| Közép        | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
...
| Property    | Default | Leírás                                                                                         |
| ----------- | ------- | ---------------------------------------------------------------------------------------------- |
| Forráskód   | Nincs   | Image or SVG to use in the painted region.                                     |
| Tint        | White   | Color filter. White at full alpha preserves the source colors. |
| Áttetszőség | 255     | Opacity of the tinted source, from `0` to `255`.                               |
| Skálázás    | `0.25`  | Size of the source pattern in the painted region.                              |
| Blur        | `0`     | Softens the painted edge.                                                      |
