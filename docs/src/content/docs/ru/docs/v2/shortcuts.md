...
| Показывать переключатель стилуса |            true, false           | Показывает быстрое переключение стиля только в редакторе после обнаружения бабочки.                                     |
|            Игнорировать давление |       Never, First, Always       | Controls whether a stylus pressure changes the stroke and works around inaccurate pressure readings from some styluses. |
