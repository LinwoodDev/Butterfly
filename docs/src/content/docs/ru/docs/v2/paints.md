...
| ------------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Цвет         | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Прозрачность | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Свойства        | По умолчанию                                                          | Описание                                                                                                                        |
| --------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Остановки цвета | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| Прозрачность    | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Свойства        | По умолчанию | Описание                                    |
| --------------- | ------------ | ------------------------------------------- |
| Начальный экран | `(0, 0)`     | Start of the gradient axis. |
| Конец           | `(1, 0)`     | End of the gradient axis.   |
...
| Свойства     | По умолчанию         | Описание                                                                      |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| По центру    | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius       | `0.5`                | Radius of the outer circle.                                   |
| Точка фокуса | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| Свойства     | По умолчанию | Описание                                                                                       |
| ------------ | ------------ | ---------------------------------------------------------------------------------------------- |
| Исходный код | Нет          | Image or SVG to use in the painted region.                                     |
| Tint         | White        | Color filter. White at full alpha preserves the source colors. |
| Прозрачность | 255          | Opacity of the tinted source, from `0` to `255`.                               |
| Масштаб      | `0.25`       | Size of the source pattern in the painted region.                              |
| Blur         | `0`          | Softens the painted edge.                                                      |
