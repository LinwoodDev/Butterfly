...
| ------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Сплошной цвет | Uses one color throughout the stroke or fill.           | Color, alpha, blur.                                |
| Градиент      | Blends between color stops.                             | Linear or radial layout, color stops, alpha, blur. |
| Изображение   | Repeats an imported raster image in the painted region. | Source image, tint, alpha, scale, blur.            |
