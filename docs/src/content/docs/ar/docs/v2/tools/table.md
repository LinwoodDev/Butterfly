...
| ----------------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| صفوف              | 3                     | Initial number of rows.                                                                                             |
| أعمدة             | 3                     | Initial number of columns.                                                                                          |
| العرض             | 0                     | Fixed table width; `0` uses the default width based on the columns.                                                 |
| الارتفاع          | 0                     | Fixed table height; `0` uses the default height based on the rows.                                                  |
| إطار              | Black, solid, width 1 | Color, width, and stroke style of table borders. Non-solid styles also expose dash and gap lengths. |
| تعبئة             | شفاف                  | Table background color and alpha.                                                                                   |
| يتغير حسب التكبير | معطل                  | Changes table creation size based on the zoom level.                                                                |
