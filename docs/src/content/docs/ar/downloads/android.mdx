...
**Legacy** apks are using the `useLegacyPackaging` flag on the build which compresses the native libraries to reduce the apk size. لكن من الممكن أن يؤثر هذا في أداء التحميل. اقرأ المزيد [في وثائق الأندرويد](https://developer.android.com/reference/tools/gradle-api/7.1/com/android/build/api/dsl/JniLibsPackagingOptions#uselegacypackaging).
