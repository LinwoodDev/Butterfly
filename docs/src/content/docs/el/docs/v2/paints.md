...
| -------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Συμπαγές χρώμα | Uses one color throughout the stroke or fill.           | Color, alpha, blur.                                |
| Διαβάθμιση     | Blends between color stops.                             | Linear or radial layout, color stops, alpha, blur. |
| Εικόνα         | Repeats an imported raster image in the painted region. | Source image, tint, alpha, scale, blur.            |
