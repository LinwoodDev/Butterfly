...
| -------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Χρώμα    | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Άλφα     | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Ιδιότητα           | Προεπιλογή                                                            | Περιγραφή                                                                                                                       |
| ------------------ | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Σταμάτημα χρώματος | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| Άλφα               | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Ιδιότητα | Προεπιλογή | Περιγραφή                                   |
| -------- | ---------- | ------------------------------------------- |
| Έναρξη   | `(0, 0)`   | Start of the gradient axis. |
| Τέλος    | `(1, 0)`   | End of the gradient axis.   |
...
| Ιδιότητα       | Προεπιλογή           | Περιγραφή                                                                     |
| -------------- | -------------------- | ----------------------------------------------------------------------------- |
| Κέντρο         | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius         | `0.5`                | Radius of the outer circle.                                   |
| Εστιακό σημείο | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| Ιδιότητα | Προεπιλογή | Περιγραφή                                                                                      |
| -------- | ---------- | ---------------------------------------------------------------------------------------------- |
| Πηγή     | Κανένα     | Image or SVG to use in the painted region.                                     |
| Tint     | White      | Color filter. White at full alpha preserves the source colors. |
| Άλφα     | 255        | Opacity of the tinted source, from `0` to `255`.                               |
| Κλίμακα  | `0.25`     | Size of the source pattern in the painted region.                              |
| Blur     | `0`        | Softens the painted edge.                                                      |
