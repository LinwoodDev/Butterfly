...
| ----------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Колір       | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Альфа       | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Властивість     | Типово                                                                | Опис                                                                                                                            |
| --------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Зупинка кольору | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| Альфа           | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Властивість | Типово   | Опис                                        |
| ----------- | -------- | ------------------------------------------- |
| Старт       | `(0, 0)` | Start of the gradient axis. |
| Кінець      | `(1, 0)` | End of the gradient axis.   |
...
| Властивість   | Типово               | Опис                                                                          |
| ------------- | -------------------- | ----------------------------------------------------------------------------- |
| Центр         | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius        | `0.5`                | Radius of the outer circle.                                   |
| Фокусна точка | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| Властивість | Типово     | Опис                                                                                           |
| ----------- | ---------- | ---------------------------------------------------------------------------------------------- |
| Джерело     | Без ефекту | Image or SVG to use in the painted region.                                     |
| Tint        | White      | Color filter. White at full alpha preserves the source colors. |
| Альфа       | 255        | Opacity of the tinted source, from `0` to `255`.                               |
