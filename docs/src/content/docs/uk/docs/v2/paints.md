...
| --------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------ |
| Суцільний колір | Uses one color throughout the stroke or fill.           | Color, alpha, blur.                                |
| Градієнт        | Blends between color stops.                             | Linear or radial layout, color stops, alpha, blur. |
| Зображення      | Repeats an imported raster image in the painted region. | Source image, tint, alpha, scale, blur.            |
