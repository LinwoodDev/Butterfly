...
|          属性 | デフォルト | 説明                              |
| ----------: | :---: | :------------------------------ |
| 描画中にカーソルを隠す |  true | 描画中にマウスポインタを隠すので、ストロークをカバーしません。 |
...
|     ジェスチャー上に移動 |  true | マルチタッチジェスチャーでノートコンテンツを操作する代わりにキャンバスを移動できます。                                                                 |
...
|         属性 |              Values              | 説明                                                                                                                               |
| ---------: | :------------------------------: | :------------------------------------------------------------------------------------------------------------------------------- |
|     ペンのみ入力 | Automatic, Always on, Always off | Prevents accidental marks from your hand or mouse when only pen input can draw.                                  |
| ペンのみのトグル表示 |            true, false           | Butterflyがペンを検出した後に、エディタにクイックペンのみのスイッチを表示します。                                                                                    |
|  プレッシャーを無視 |       Never, First, Always       | Controls whether a pen pressure changes the stroke and works around inaccurate pressure readings from some pens. |
