...
| 属性   | デフォルト                                    | 説明                                                                                                      |
| ---- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 色    | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| アルファ | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| 属性   | デフォルト                                                                 | 説明                                                                                                                              |
| ---- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 色の停止 | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| アルファ | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| 終了 | `(1, 0)` | End of the gradient axis.   |
...
| 属性           | デフォルト                | 説明                                                                            |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| 中央揃え         | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius       | `0.5`                | Radius of the outer circle.                                   |
| 焦点点          | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| 属性   | デフォルト  | 説明                                                                                             |
| ---- | ------ | ---------------------------------------------------------------------------------------------- |
| ソース  | なし     | Image or SVG to use in the painted region.                                     |
| Tint | White  | Color filter. White at full alpha preserves the source colors. |
| アルファ | 255    | Opacity of the tinted source, from `0` to `255`.                               |
| 拡大縮小 | `0.25` | Size of the source pattern in the painted region.                              |
| Blur | `0`    | Softens the painted edge.                                                      |
