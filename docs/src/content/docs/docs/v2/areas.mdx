...
import {Monitor, TextT, Faders, Folder, Stack, SignOut, SignIn, Trash, Export, PlusCircle, Door, DotsThreeVertical} from "@phosphor-icons/react";
...
## Creating Areas
 ...
 With the Area tool selected, you can draw a rectangle anywhere on the canvas to define a new area. If you do not have the <Monitor className="inline-icon"/> Area tool on your toolbar, you can follow [these instructions](../intro#important-notes) to add it.
...
You can configure the Area tool by selecting the <Monitor className="inline-icon"/> Area tool in the toolbar and then clicking it again. The tool menu has the following options:
- `Ask for name` - Set whether or not to prompt for a name when a new area is created
- `Width` - Specify a fixed width for new areas. If `0`, this setting will be ignored.
- `Height` - Specify a fixed height for new areas. If `0`, this setting will be ignored.
- `Aspect ratio` - Specify a fixed aspect ratio for new areas. Press the <DotsThreeVertical className="inline-icon"/> button to access some common presets. Values less than `1` will be taller than they are wide, and values greater than `1` will be wider than they are tall. If `0`, this setting will be ignored.