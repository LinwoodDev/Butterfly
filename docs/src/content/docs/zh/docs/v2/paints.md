...
| 属性   | 默认设置                                     | 描述                                                                                                      |
| ---- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| 颜色   | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| 不透明度 | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| 属性   | 默认设置                                                                  | 描述                                                                                                                              |
| ---- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 颜色停止 | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| 不透明度 | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| 属性 | 默认设置     | 描述                                          |
| -- | -------- | ------------------------------------------- |
| 开始 | `(0, 0)` | Start of the gradient axis. |
| 结束 | `(1, 0)` | End of the gradient axis.   |
...
| 属性           | 默认设置                 | 描述                                                                            |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| 居中           | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius       | `0.5`                | Radius of the outer circle.                                   |
| 协调中心         | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| 属性   | 默认设置   | 描述                                                                                             |
| ---- | ------ | ---------------------------------------------------------------------------------------------- |
| 源代码  | 无      | Image or SVG to use in the painted region.                                     |
| Tint | White  | Color filter. White at full alpha preserves the source colors. |
| 不透明度 | 255    | Opacity of the tinted source, from `0` to `255`.                               |
| 缩放   | `0.25` | Size of the source pattern in the painted region.                              |
| Blur | `0`    | Softens the painted edge.                                                      |
