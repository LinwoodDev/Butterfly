...
在这里你可以看到项目中的所有图层。 To create a new layer, use the Create button. To
...
|    选项 | 描述                                                                                |
| ----: | --------------------------------------------------------------------------------- |
|   前进中 | Renders the selection on top of the first element in front of it. |
|  向后发送 | Renders the selection under the first element behind it.          |
|  置于前端 | Renders the selection in front of everything else.                |
| 发送到背面 | Renders the selection behind everything else.                     |
