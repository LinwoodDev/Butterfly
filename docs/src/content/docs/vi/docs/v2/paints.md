...
| ------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Màu sắc       | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Độ trong suốt | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Property      | Default                                                               | Mô tả                                                                                                                           |
| ------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Color stops   | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| Độ trong suốt | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Property | Default  | Mô tả                                       |
| -------- | -------- | ------------------------------------------- |
| Bắt đầu  | `(0, 0)` | Start of the gradient axis. |
| End      | `(1, 0)` | End of the gradient axis.   |
...
| Property     | Default              | Mô tả                                                                         |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| Giữa         | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
...
| Property      | Default  | Mô tả                                                                                          |
| ------------- | -------- | ---------------------------------------------------------------------------------------------- |
| Nguồn         | Không có | Image or SVG to use in the painted region.                                     |
| Tint          | White    | Color filter. White at full alpha preserves the source colors. |
| Độ trong suốt | 255      | Opacity of the tinted source, from `0` to `255`.                               |
| Tỷ lệ         | `0.25`   | Size of the source pattern in the painted region.                              |
| Blur          | `0`      | Softens the painted edge.                                                      |
