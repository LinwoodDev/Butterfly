...
| ------------- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Cor           | Black for strokes; transparent for fills | Color of the painted region.                                                            |
| Transparência | 255 for strokes; 0 for fills             | Opacity from transparent (`0`) to opaque (`255`). |
...
| Propriedade   | Padrão                                                                | Descrição                                                                                                                       |
| ------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Cor de parada | Black at `0`; white at `1`                                            | Each stop has an offset, color, and alpha value. The offset determines where the color appears. |
| Transparência | 255 for a new opaque stroke; 0 when converted from a transparent fill | Changes the opacity of all stops, from `0` to `255`.                                                            |
...
| Propriedade | Padrão   | Descrição                                   |
| ----------- | -------- | ------------------------------------------- |
| Iniciar     | `(0, 0)` | Start of the gradient axis. |
| Término     | `(1, 0)` | End of the gradient axis.   |
...
| Propriedade  | Padrão               | Descrição                                                                     |
| ------------ | -------------------- | ----------------------------------------------------------------------------- |
| Centralizar  | `(0.5, 0.5)`         | Center of the outer circle, relative to the element bounds.   |
| Radius       | `0.5`                | Radius of the outer circle.                                   |
| Ponto focal  | Off; uses the center | Enable to move the inner starting point away from the center. |
...
| Propriedade   | Padrão  | Descrição                                                                                      |
| ------------- | ------- | ---------------------------------------------------------------------------------------------- |
| Fonte         | Nenhuma | Image or SVG to use in the painted region.                                     |
| Tint          | White   | Color filter. White at full alpha preserves the source colors. |
| Transparência | 255     | Opacity of the tinted source, from `0` to `255`.                               |
| Escala        | `0.25`  | Size of the source pattern in the painted region.                              |
| Blur          | `0`     | Softens the painted edge.                                                      |
