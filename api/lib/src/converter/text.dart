...
Archive convertTextDataToArchive(Map<String, dynamic> data) {
  ...
  for (final page
      in (data['pages'] as Map<String, dynamic>?)?.entries ??
          <MapEntry<String, dynamic>>[]) {
    reader = reader.setAsset(
      '$kPagesArchiveDirectory/${page.key}.json',
  ...
  final assets =
      (data['assets'] as Map<String, dynamic>?)?.entries ??
      <MapEntry<String, dynamic>>[];
...
Map<String, dynamic> convertDocumentToText(NoteData data) {
  ...
  if (thumbnail != null) {
    output['thumbnail'] = UriData.fromBytes(
      thumbnail,
      mimeType: 'image/png',
    ).toString();
  }
