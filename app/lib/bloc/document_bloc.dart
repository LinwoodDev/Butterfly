...
class DocumentBloc extends ReplayBloc<DocumentEvent, DocumentState> {
  ...
  void _init() {
        ...
        for (var renderer in current.renderers) {
          ...
          if (updated != null) {
            for (var element in updated) {
              ...
              await newRenderer.setup(current.page);
    ...
    on<DocumentBackgroundChanged>((event, emit) async {
      if (state is DocumentLoadSuccess) {
        ...
        await background.setup(current.page);
        await _saveState(
            emit,
            current.copyWith(
                page: current.page.copyWith(
    ...
    on<LayerRenamed>((event, emit) async {
      if (state is DocumentLoadSuccess) {
        ...
        await Future.wait(
            renderer.map((e) async => await e.setup(current.page)));
        await _saveState(
            emit,
            current.copyWith(
      ...
      if (state is DocumentLoadSuccess) {
        ...
        final renderers = await Future.wait(
          ...
          if (e.element.layer == event.name) {
            ...
            await renderer.setup(current.page);
            return renderer;
          }
          return e;
        }));
  ...
  void refresh() {
    final current = state;
    if (current is! DocumentLoadSuccess) return;
    current.currentIndexCubit.refresh(current.page);
  }
